...
{   
    ...
    private function rule($request,$bool = false){
        ...
        'none'=>['nullable'=>['状态可以为空'],'regex:@^1$@'=>['状态不正确']],
        'super_model'=>['nullable'=>['状态可以为空'],'regex:@^1$@'=>['状态不正确']],
    ...
    public function store(Request $request)
    {
     ...
     $data['none'] = $request -> input('none',0);
     $data['super_model'] = $request -> input('super_model',0);
    ...
    public function edit($id)
    {
        ...
        ->where('id',$id)
        ->where('id','<>','1')
        ->where('id','<>',session('admin_user_id'))->first();
        if(empty($r->id)){return L();}
        $row = DB::table($this->_DBT)->where('state','=',1)->select('id','name')->get();
    ...
    public function update(Request $request, $id)
    {
        ...
        ->where('id','=',$id)
        ->where('id','<>','1')
        ->where('id','<>',session('admin_user_id'))->update($data);
    ...
    {
      $message = DB::table($this->_DB)->where('id','=',$id)->where('id','<>','1')->where('id','<>',session('admin_user_id'))->delete();
     ...
     function state(Request $r,$id){
         ...
         $message = DB::table($this->_DB)->where('id',$id)->where('id','<>','1')->where('id','<>',session('admin_user_id'))->update(['state'=>$value]);
         update_log($r,$message,$this->_Log,$this->_DB,['state'=>$value],$id);
         return ['error'=> $message?0:1];
    }
      function none(Request $r,$id){
         $value = $v = $r->input('value',1);
         $message = DB::table($this->_DB)->where('id',$id)->where('id','<>','1')->where('id','<>',session('admin_user_id'))->update(['none'=>$value]);
         update_log($r,$message,$this->_Log,$this->_DB,['none'=>$value],$id);
         return ['error'=> $message?0:1];
    }
      function super(Request $r,$id){
         $value = $v = $r->input('value',1);
         $message = DB::table($this->_DB)->where('id',$id)->where('id','<>','1')->where('id','<>',session('admin_user_id'))->update(['super_model'=>$value]);
         update_log($r,$message,$this->_Log,$this->_DB,['super_model'=>$value],$id);
         return ['error'=> $message?0:1];
    }
    function resetpass(Request $r,$id){
         ...
         $message = DB::table($this->_DB)->where('id',$id)->where('id','<>','1')->where('id','<>',session('admin_user_id'))->update($update);
