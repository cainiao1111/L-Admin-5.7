(function() {
	...
	! function(e) {
		e.RMenu = {
			init: function(per) {
				if(typeof(per) != "object" && !per.hasOwnProperty('area') && !per.hasOwnProperty('items') && !per.hasOwnProperty('callback')) throw "json 鏁版嵁閿欒";
				that = this;
				this.showmenu(per);
			...
			contextMenu: function(per, key) {
				...
				$.each(per.items, function(item, val) {
					var icon = val.icon ? '<i class="fa  iconfont  fa-fw ">' + val.icon : ''
