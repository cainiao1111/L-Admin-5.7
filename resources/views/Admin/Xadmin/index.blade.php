      ...
      <table class="layui-table layui-form">
        ...
        <tbody>
          ...
          <tr class="dom_item_{{$v->id}}">
            ...
            <td>
   			<input type="checkbox" data-id="{{$v->id}}" lay-filter="super_model" name="super_model" @if($v->super_model == 1) checked="checked" @endif  lay-skin="switch"   lay-text="是|否">
            </td>
            <td>
...
//super	
form.on('switch(super_model)', function(data){
if($req)return false;
var $this = $(this),id = $this.attr('data-id');
$this.attr('checked' ,!$this.attr('checked'));
var $arr =['是','否'],value = $this.attr('checked')?1:0;
...
layer.load();
 ...
 $.post("adminapi/{{$d['mcm']['controller']}}/super_"+id,{value:value,_method:'PUT'},function(e){		
 		 layer.msg(e.error == 0?'修改成功!':'修改失败!');	
 	    $req = false; 	
 	    layer.closeAll('loading');	
 	    if(!e.error == 0){
